...
copies_16s <- 
  ...
  mutate(
    ...
    across(.cols = contains(c("16s", "anaerobe")), as.numeric)
  ) %>% 
  mutate(
    rRNA_16S_copy_number_per_g = copy_number_16s_rrna_per_kg / 1000,
    rRNA_16S_log_copies_per_g = log(rRNA_16S_copy_number_per_g),
    sd_copies_per_g = sd_copy_number_16s / 1000,
    sd_log_copies_per_g = log(sd_copies_per_g)
  )
...
all_sample_data %>% 
  filter(!is.na(rRNA_16S_copy_number_per_g_soil_logtrans)) %>% View()
  ggplot(
    aes(
      x = distance_mm, 
